...
/*   Updated: 2019/03/20 19:43:20 by dheredat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_sqr_test(char **map_array, int i, int j)
{
	...
	counter = 1;
	while ((j+counter < t_head.length) && (i+counter < t_head.size))
	{
		step = 0;
		while(step <= counter)
		{
...
int ft_stalker(char **map_array)
{
	...
	printf("t_head.size %d\n", t_head.size);
	printf("t_head.length %d\n", t_head.length);
//	ft_putchar('1');
	i = 0;
	sqr_max = 0;
	sqr_cur = 0;
